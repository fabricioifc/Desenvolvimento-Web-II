...
\begin{frame}
	...
	\framesubtitle{Monolito}
	
	Abordagem tradicional (Figura \ref{fig:monolito}) no desenvolvimento de software na qual todos os componentes de uma aplicação são combinados em uma única unidade totalmente integrada. A aplicação é implantada como uma única e grande base de código que contém todas as funcionalidades.

	\begin{figure}
		\centering
		\includegraphics[width=0.4\linewidth]{Images/monolito.jpg}
		\caption{Arquitetura Monolítica teste.}\label{fig:monolito}
	\end{figure}
...
\begin{frame}
	...
	\begin{exampleblock}{Prós}
		\begin{itemize}
			\item \textbf{Simples de entender e desenvolver}: os componentes são combinados em uma única base de código.
			\item \textbf{Fácil de testar e implantar}: a aplicação é implantada como uma única unidade.
		\end{itemize}
	\end{exampleblock}
	
	\begin{alertblock}{Contras}
		\begin{itemize}
			\item \textbf{Desafios de escalabilidade}: a aplicação é implantada como uma única unidade, tornando difícil escalar componentes individuais.
			\item \textbf{Modularidade limitada}: as aplicações monolíticas são totalmente integradas, dificultando a alteração de recursos existentes sem afetar outros componentes.
			\item \textbf{Falta de agilidade}: dificultando o trabalho das equipes em diferentes componentes em paralelo.
		\end{itemize}
	\end{alertblock}

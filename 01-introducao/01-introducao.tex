...
\begin{frame}
	...
	\begin{itemize}
		\item \textbf{Componentes}: partes que compõem a aplicação web.
		\item \textbf{Conectores}: mecanismos que permitem a comunicação entre os componentes.
		\item \textbf{Restrições}: regras que definem como os componentes e conectores podem interagir.
	\end{itemize}

	\begin{block}{Web Moderna}
		A arquitetura moderna da Web enfatiza a \alert{escalabilidade} das interações entre componentes, a \alert{generalidade} das interfaces, a implantação \alert{independente} de componentes e a presença de componentes intermediários para reduzir a latência da interação, reforçar a segurança e encapsular sistemas legados \parencite{Fielding2002}.
	\end{block}
